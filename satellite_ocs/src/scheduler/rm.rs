...
//     let mut tasks = vec![
//         RtTask::new("navigation_update",  50, 3.0, 1, now),   
//         RtTask::new("data_compression",  100, 6.0, 2, now),   
//         RtTask::new("health_monitor",   1000, 2.0, 3, now),   
//     ];
...
//         // FIXED LOGIC: Dynamic sleep calculation to avoid OS timer resolution bugs
//         while job.remaining_ms > 0.0 {
//             let current_prio = if spec_is_thermal { 0 } else { tasks[job.task_idx].rm_priority };
...
//             let run_for_ms = ms_to_next_preemption.max(0.1); // Prevent 0ms locks

//             // Use tokio::select! to instantly preempt if thermal control fires
//             tokio::select! {
//                 _ = time::sleep(Duration::from_micros((run_for_ms * 1000.0) as u64)) => {
//                     job.remaining_ms -= run_for_ms;
//                     active_ms_acc += run_for_ms;
//                 }
//                 _ = rx_preempt.recv() => {
