...
pub enum EventType {
    ...
    FaultRecoveryMetric, // metadata: mttr_avg_ms, avg_fault_response_time_ms
}
...
impl PerformanceTracker {
    pub fn new() -> Self {
        info!("Initializing Performance Tracking Engine");
    ...
    pub fn record_performance_event(&mut self, perf_event: PerformanceEvent) {
        ...
        self.apply_event_metrics(&perf_event);
        
        // Check for performance violations
        self.detect_performance_violations(&perf_event);
    ...
    fn apply_event_metrics(&mut self, event: &PerformanceEvent) {
        match event.event_type {
            ...
            EventType::CommandResponseRttSample => {
                self.command_response_rtt_times.push_back(event.duration_ms);
                if self.command_response_rtt_times.len() > 1000 {
            ...
            EventType::TaskExecutionDrift => {
                ...
                // Keep counting threshold violations for metrics/reporting, but avoid
                // logging every scheduler tick to prevent noisy output.
            ...
            EventType::FaultRecoveryMetric => {
                if let Some(mttr_s) = event.metadata.get("mttr_avg_ms") {
                    if let Ok(mttr) = mttr_s.parse::<f64>() {
                        self.fault_recovery_mttr_samples_ms.push_back(mttr);
                        if self.fault_recovery_mttr_samples_ms.len() > 1000 {
                            self.fault_recovery_mttr_samples_ms.pop_front();
                        }
                    }
                }
                if let Some(resp_s) = event.metadata.get("avg_fault_response_time_ms") {
                    if let Ok(resp) = resp_s.parse::<f64>() {
                        self.fault_response_avg_samples_ms.push_back(resp);
                        if self.fault_response_avg_samples_ms.len() > 1000 {
                            self.fault_response_avg_samples_ms.pop_front();
                        }
                    }
                }
            }
    ...
    fn detect_performance_violations(&mut self, event: &PerformanceEvent) {
        match event.event_type {
            EventType::TelemetryProcessed => {
                if event.duration_ms > 3.0 {
    ...
    fn prune_old_timing_data(&mut self) {
        ...
        if self.command_response_rtt_times.len() > max_timing_samples {
            self.command_response_rtt_times.pop_front();
        }
        ...
        if self.fault_response_avg_samples_ms.len() > max_timing_samples {
            self.fault_response_avg_samples_ms.pop_front();
        }
    ...
    pub fn snapshot_current_stats(&self) -> PerformanceStats {
        ...
        let p95_processing_time = self.compute_percentile(&self.telemetry_processing_times, 95.0);
        let p99_processing_time = self.compute_percentile(&self.telemetry_processing_times, 99.0);

        let (avg_command_response_rtt_ms, p95_command_response_rtt_ms, p99_command_response_rtt_ms, max_command_response_rtt_ms)
            = if !self.command_response_rtt_times.is_empty() {
                let sum = self.command_response_rtt_times.iter().sum::<f64>();
                let avg = sum / self.command_response_rtt_times.len() as f64;
                let p95 = self.compute_percentile(&self.command_response_rtt_times, 95.0);
                let p99 = self.compute_percentile(&self.command_response_rtt_times, 99.0);
                let max = self.command_response_rtt_times.iter().cloned().fold(0.0, f64::max);
                (avg, p95, p99, max)
            } else {
                (0.0, 0.0, 0.0, 0.0)
            };
        ...
        let (avg_task_drift, max_task_drift, p95_task_drift, p99_task_drift) = if !self.task_drift_ms.is_empty() {
            ...
            let p95 = self.compute_percentile(&self.task_drift_ms, 95.0);
            let p99 = self.compute_percentile(&self.task_drift_ms, 99.0);
            (avg, max, p95, p99)
        } else {
            (0.0, 0.0, 0.0, 0.0)
        };
        ...
        let backlog_p95_len = self.compute_percentile(&backlog_len_f64, 95.0);
        ...
        let backlog_p95_age_ms = self.compute_percentile(&self.telemetry_backlog_age_ms, 95.0);
        ...
        PerformanceStats {
            ...
            uptime_percentage: self.compute_uptime_percentage(),
            ...
            avg_command_response_rtt_ms,
            p95_command_response_rtt_ms,
            p99_command_response_rtt_ms,
    ...
    fn collect_recent_events(&self, duration: Duration) -> Vec<&PerformanceEvent> {
    ...
    fn compute_percentile(&self, values: &VecDeque<f64>, percentile: f64) -> f64 {
        if values.is_empty() {
            return 0.0;
        }
    ...
    fn compute_uptime_percentage(&self) -> f64 {
    ...
    fn compute_system_health_score(&self) -> f64 {
        ...
        if !self.task_drift_ms.is_empty() {
            let avg_task_drift_ms =
                self.task_drift_ms.iter().sum::<f64>() / self.task_drift_ms.len() as f64;
            let p99_task_drift_ms = self.compute_percentile(&self.task_drift_ms, 99.0);
    ...
    pub fn build_performance_report(&self, duration: Duration) -> PerformanceReport {
        let recent_events = self.collect_recent_events(duration);
        let stats = self.snapshot_current_stats();
...
pub struct PerformanceStats {
    ...
    pub avg_command_response_rtt_ms: f64,
    pub p95_command_response_rtt_ms: f64,
    pub p99_command_response_rtt_ms: f64,
